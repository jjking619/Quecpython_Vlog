...
if __name__ == '__main__':
    ...
    while True:
            ...
            print("Humidity: %.2f, Temperature: %.2f"%(hum,tem))
            topic='/sys/k28gtnnq6qN/aliyun_demo/thing/event/property/post'
            # 订阅主题
            ali.subscribe(topic)
            # 发布消息
            ali.publish(topic, '{"method" : "thing.event.property.post", "params" : {"temperature" : %.2f, "Humidity":%.2f}, "version" : "1.0.0"}'%(tem,hum))
            # 运行
            ali.start()
            utime.sleep(2)
